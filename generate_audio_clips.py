...
CLIPS = [
    ...
    ("water_medium",  "Water Level Sixty One Percent", "Water Level Sixty One Percent"),
    ("tank_full",     "Tank Full",                     "Tank Full"),
]
...
def tts_to_pcm(phrase: str) -> bytes:
    """Generate speech for phrase and return 16-bit mono PCM bytes."""
    from gtts import gTTS

    print(f"       Fetching TTS from Google...")
    buf = io.BytesIO()
    gTTS(text=phrase, lang="en", slow=False).write_to_fp(buf)
 ...
 *   ffmpeg -i recording.mp3 -ar {sample_rate} -ac 1 -f s16le out.raw
...
def build_c_file(clip_data):
    ...
    parts = []
    for name, samples, label in clip_data:
        sep = (
            f"/* -------------------------------------------------------------------\n"
            f" * \"{label}\"\n"
...
def main():
    ...
    total_samples = sum(len(s) for _, s, _ in clip_data)
